...
def _build_schedule_schema_from_db(week_id: UUID, user_id: UUID, db: Session):
    shifts = db.query(Shift).filter(Shift.week_id == week_id,
    ...
    for shift in shifts:
        ...
        for assignment in assignments:
            ...
            schedule_shift_employees_out.append(
                schemas.ScheduleShiftEmployeeOut(employee_id=employee.id, name=str(employee.name)))
        schedule_shifts_out.append(schemas.ScheduleShiftOut(shift_id=shift.id,
